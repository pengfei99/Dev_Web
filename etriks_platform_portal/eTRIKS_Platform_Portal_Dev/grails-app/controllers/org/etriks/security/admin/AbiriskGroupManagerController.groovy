...
@Transactional
class AbiriskGroupManagerController {
    //database log
    def AccessLogService accessLogService;
    //create group manager service
    ...
    def createGroupLdapEntry(){
        ...
        //Create group in old ldap archi
        ...
        if (result){
            ...
            accessLogService.saveLog(messageTmp,session.userDetails.username, ProjectId.abirisk, LogType.Info);
            flash.message=messageTmp
            render(view: "createGroup")
        }
        else {
            accessLogService.saveLog("Group "+groupName+" cannot be created !!! Please check if the group name has been used",session.userDetails.username, ProjectId.abirisk, LogType.Error);
            ...
            if(!chosenUserList.isEmpty()){
                ...
                def resultMessage=groupManagerService.addMemberToPosixGroupInNewLdap(projectName,chosenUserList,chosenGroupList)
                  for(String message:resultMessage){
                       accessLogService.saveLog(message,session.userDetails.username, ProjectId.abirisk, LogType.Info);
                }
