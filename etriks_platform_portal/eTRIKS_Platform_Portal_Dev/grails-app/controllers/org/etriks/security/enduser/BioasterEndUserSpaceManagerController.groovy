...
class BioasterEndUserSpaceManagerController {

    def AccessLogService accessLogService;
    //Define the bioaster ldap profil instance
    def PubLdapConnectorConfig prodLdap=new PubLdapConnectorConfig();
    ...
    def resetLdapUserAccountPassword(){
        ...
        //user account manager for new ldap archi
        ...
        accessLogService.saveLog(clientIp+" wants to reset password of "+uid,clientIp,ProjectId.bioaster,LogType.Info)
        if(userDn==null){
            flash.error="The uid which you entered does not exist!!!"
            accessLogService.saveLog(clientIp+" reset password of "+uid+" failed, uid doesnot exist",clientIp,ProjectId.bioaster,LogType.Info)
            ...
            if(mail.equals(email)){
                ...
                if(result){
                    ...
                    String msg=new MailMessage().getResetPasswordMessage("bioaster", uid,pwd1);
                    try {
                    ...
                    accessLogService.saveLog(clientIp+" reset the password of "+uid+" successfully",clientIp,ProjectId.bioaster,LogType.Info)
                ...
                //reset password fail, report to error log
                    ...
                    log.error("Reset user password failed in controller: BioasterEndUserSpaceManager, action :resetLdapUserAccountPassword ");
                    render(view:"resetPassword")
                }
            ...
            //The entered email address dosenot match the registered email address, stop the reset. alert maybe an attack
                ...
                accessLogService.saveLog(clientIp+" reset password of "+uid+" failed, email doesnot match",clientIp,ProjectId.bioaster,LogType.Info)
                render(view:"resetPassword")
    ...
    def changeLdapUserAccountPassword(String uid){
        ...
        if (pwdStrengthLevel<3){
            ...
            if(newPassword.equals(confNewPassword)){
                ...
                //Create an instance of the end user account manager in new ldap archi,
                ...
                if(result.equals(ChangePasswordStatus.Success)){
                    ...
                    accessLogService.saveLog(LogMessages.getChangePwdMessage(uid),userInstance.username,ProjectId.bioaster,LogType.Info)
                    redirect(action:"index")
                }
                if(result.equals(ChangePasswordStatus.UidNotFind)){
                    accessLogService.saveLog("Change password failed, account name does not exist",userInstance.username,ProjectId.bioaster,LogType.Info)
                ...
                if(result.equals(ChangePasswordStatus.WrongPassword)){
                    accessLogService.saveLog("Change password failed, old password is wrong",userInstance.username,ProjectId.bioaster,LogType.Info)
                    flash.message="Your password is wrong,contact your admin for more information"
                    redirect(action:"changePassword")
                }
                if(result.equals(ChangePasswordStatus.Fail)){
                    accessLogService.saveLog("Change password failed, Ldap server is not responding",userInstance.username,ProjectId.bioaster,LogType.Error)
                    flash.message="The server is not responding, your password cannot be changed. Please try later"
                    redirect(action:"index")
                }
            }
            else {
                accessLogService.saveLog("Change password failed, confirm new password and new password are different",userInstance.username,ProjectId.bioaster,LogType.Info)
