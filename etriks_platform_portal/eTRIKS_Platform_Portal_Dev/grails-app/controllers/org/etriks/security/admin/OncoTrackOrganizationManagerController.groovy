...
@Transactional
class OncoTrackOrganizationManagerController {

   /* def OncoTrackLdapConnectorConfig oncoTrackLdap=new OncoTrackLdapConnectorConfig();*/

    private String projectName="oncotrack";
    ...
    def createOrganizationLdapEntry(){
        ...
        if (result){
            accessLogService.saveLog("Organization "+organizationName+" has been created in the Ldap server",session.userDetails.username, ProjectId.oncoTrack, LogType.Info)
            flash.message="Organization "+organizationName+" has been created in the Ldap server";
                     render(view: "createOrganization")}
        else {
            accessLogService.saveLog("Organization "+organizationName,session.userDetails.username, ProjectId.oncoTrack, LogType.Error)
        ...
        if(!chosenUserList.isEmpty()){
            for(int j=0;j<chosenUserList.size();j++){
                ...
                if (result){
                    ...
                    accessLogService.saveLog("User "+uid+" is now member of the "+orgName,session.userDetails.username, ProjectId.oncoTrack, LogType.Info)
                }
                else {
                    accessLogService.saveLog("User "+uid+" can not change his organization ",session.userDetails.username, ProjectId.oncoTrack, LogType.Error)
