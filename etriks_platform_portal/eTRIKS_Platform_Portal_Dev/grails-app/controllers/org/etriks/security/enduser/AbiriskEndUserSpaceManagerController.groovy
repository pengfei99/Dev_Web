...
@Transactional
class AbiriskEndUserSpaceManagerController {


        def AccessLogService accessLogService;
    //Define the abirisk ldap profil instanc
    ...
    def resetLdapUserAccountPassword(){
        ...
        accessLogService.saveLog(clientIp+" wants to reset password of "+uid,clientIp,ProjectId.abirisk,LogType.Info)
        if(userDn==null){
            flash.error="The uid which you entered does not exist!!!"
            accessLogService.saveLog(clientIp+" reset password of "+uid+" failed, uid doesnot exist",clientIp,ProjectId.abirisk,LogType.Info)
            ...
            if(mail.equals(email)){
                ...
                if(result){
                  ...
                  flash.message="Your password has been reset, you will receive your new password by email. You need to change it as soon as possible "
                    ...
                    try {
                        ...
                        log.error("unable to send user the reset password notification email in controller: AbiriskEndUserSpaceManager, action :resetLdapUserAccountPassword. Java exception "+e.toString());
                    }
                    accessLogService.saveLog(clientIp+" reset the password of "+uid+" successfully",clientIp,ProjectId.abirisk,LogType.Info)
                ...
                //reset password fail, report to error log
                    ...
                    log.error("Reset user password failed in controller: AbiriskEndUserSpaceManager, action :resetLdapUserAccountPassword ");
                    render(view:"resetPassword")
                }
            ...
            //The entered email address dosenot match the registered email address, stop the reset. alert maybe an attack
                ...
                accessLogService.saveLog(clientIp+" reset password of "+uid+" failed, email doesnot match",clientIp,ProjectId.abirisk,LogType.Info)
                render(view:"resetPassword")
        ...
        def changeLdapUserAccountPassword(String uid){
            ...
            if(newPassword.equals(confNewPassword)){
                ...
                if(result.equals(ChangePasswordStatus.Success)){
                    ...
                    accessLogService.saveLog(LogMessages.getChangePwdMessage(uid),userInstance.username,ProjectId.abirisk,LogType.Info)
                    redirect(action:"index")
                }
                if(result.equals(ChangePasswordStatus.UidNotFind)){
                    accessLogService.saveLog("Change password failed, account name does not exist",userInstance.username,ProjectId.abirisk,LogType.Info)
                ...
                if(result.equals(ChangePasswordStatus.WrongPassword)){
                    accessLogService.saveLog("Change password failed, old password is wrong",userInstance.username,ProjectId.abirisk,LogType.Info)
                    flash.message="Your password is wrong,contact your admin for more information"
                    redirect(action:"changePassword")
                }
                if(result.equals(ChangePasswordStatus.Fail)){
                    accessLogService.saveLog("Change password failed, Ldap server is not responding",userInstance.username,ProjectId.abirisk,LogType.Error)
                    flash.message="The server is not responding, your password cannot be changed. Please try later"
                    redirect(action:"index")
                }
            }
            else {
                accessLogService.saveLog("Change password failed, confirm new password and new password are different",userInstance.username,ProjectId.abirisk,LogType.Info)
