...
@Transactional
class OncoTrackGroupManagerController {
    //oncoTrack Ldap Server connection config instance
    ...
    def createGroupLdapEntry(){
        ...
        if (result){
            accessLogService.saveLog("Group "+groupName+" has been created in the Ldap server",session.userDetails.username, ProjectId.oncoTrack, LogType.Info);
            flash.message="Group "+groupName+" has been created in the Ldap server";
            render(view: "createGroup")
        }
        else {
            accessLogService.saveLog("Group "+groupName+" cannot be created !!! Please check if the group name has been used",session.userDetails.username, ProjectId.oncoTrack, LogType.Error);
   ...
   if(!chosenGroupList.isEmpty()){
       if(!chosenUserList.isEmpty()){
           ...
           for(String message:resultMessage){
               accessLogService.saveLog(message,session.userDetails.username, ProjectId.oncoTrack, LogType.Info);
           }
    ...
    /*def addMemberToGroupResult(){
        def resultMessage=params.get("resultMessage");
