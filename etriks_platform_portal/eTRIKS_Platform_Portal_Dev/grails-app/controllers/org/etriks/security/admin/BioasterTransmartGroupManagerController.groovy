...
class BioasterTransmartGroupManagerController {
            ...
            if(!chosenUserList.isEmpty()){
                ...
                for(int i=0;i<chosenGroupList.size();i++){
                    ...
                    for(int j=0;j<chosenUserList.size();j++){
                        ...
                        if(result){
                            accessLogService.saveLog("User "+uid+" has been added to group "+groupName,session.userDetails.username, ProjectId.bioaster, LogType.Info);
                            resultMessage.add("User "+uid+" has been added to group "+groupName)
                        }
                        else {
                            accessLogService.saveLog("User "+uid+" cannot been added to group "+groupName,session.userDetails.username, ProjectId.bioaster, LogType.Error);
